...
fn bench_file_throughput(c: &mut Criterion) {
    ...
    // Write a file once for the read benchmarks
    ...
    group.bench_function("read_mmap_warm", |b| {
        ...
        b.iter(|| {
            ...
            for i in 0..reader.num_batches() {
                ...
                // Touch column data to prevent optimisation
                std::hint::black_box(&batch.cols);
            }
        });
    });
    ...
    // arrow-rs file reader for comparison
    #[cfg(feature = "bench_arrow")]
    group.bench_function("read_arrow_rs_file", |b| {
