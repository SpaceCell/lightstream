...
pub fn resolve_source(
    ...
    if out_of_core && mmap == Some(false) {
        return Err(PyValueError::new_err(
            "out_of_core applies to the mmap reader, so it cannot combine with mmap=False",
        ));
    }
    ...
    let file_io = match resolved_format {
        Format::Ipc => {
            ...
            if use_mmap {
                let reader = MmapTableReader::open(&path, out_of_core)
                    .map_err(|e| FormatError::new_err(e.to_string()))?;
