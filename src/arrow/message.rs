...
///
/// The tag values below match the member positions in the official Arrow
/// Type union. Types Minarrow does not support are omitted, so every new
/// member must state its official tag value.
#[derive(Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Default)]
#[repr(transparent)]
...
impl Type {
  ...
  pub const Utf8View: Self = Self(23);
...
///
/// The tag values match the member positions in the official Arrow
/// MessageHeader union, which also carries Tensor = 4 and SparseTensor = 5.
#[derive(Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Default)]
#[repr(transparent)]
